...
const getFallbackColor = (
  ...
  return newRgb.map((v) => Math.round((v + m) * 255)) as RGB
}

export const getColors = (img: HTMLImageElement, minContrast = 4.5) => {
  const primaryColor = Thief.getColor(img)
  let secondaryColor: RGB, tercaryColor: RGB
  ...
  let palette = (Thief.getPalette(img) as RGB[]).map((swatch) => [
    ...swatch,
    luminance(...swatch),
  ])
  palette = palette
  ...
  for (const swatch of palette as RGB[]) {
  ...
  return {
    ...
    secondaryColor: rgbToHex(
      // @ts-ignore
      secondaryColor[0], // @ts-ignore
      secondaryColor[1], // @ts-ignore
      secondaryColor[2]
    ), // @ts-ignore
